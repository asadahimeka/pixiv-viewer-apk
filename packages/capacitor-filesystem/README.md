...
```typescript
getFileSize(options: StatOptions) => Promise<{ size: number; len: number; }>
```
...
**Returns:** <code>Promise&lt;{ size: number; len: number; }&gt;</code>
...
| --------------------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| **`Downloads`**       | <code>'DOWNLOADS'</code>        | The Downloads directory. On iOS it's not supported. Use this directory to store user-generated content. On Android it's the Public Downloads folder, so it's accessible from other apps. It's not accesible on Android 10 unless the app enables legacy External Storage by adding `android:requestLegacyExternalStorage="true"` in the `application` tag in the `AndroidManifest.xml`. On Android 11 or newer the app can only access the files/folders the app created.                 | 5.1.2 |
| **`Pictures`**        | <code>'PICTURES'</code>         | The Pictures directory. On iOS it's not supported. Use this directory to store user-generated content. On Android it's the Public Pictures folder, so it's accessible from other apps. It's not accesible on Android 10 unless the app enables legacy External Storage by adding `android:requestLegacyExternalStorage="true"` in the `application` tag in the `AndroidManifest.xml`. On Android 11 or newer the app can only access the files/folders the app created.                   | 5.1.2 |
